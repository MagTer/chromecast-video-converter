...
3. **Job creation** — `record_library_entry` checks for an existing `*-chromecast.mp4`. If conversion is required, `JobManager` enqueues a Redis job with the selected profile encoding payload. While a convert job is in flight the entry stays `converting` and no compliance verify is queued. Manual scans via `/api/scan` walk the mounted root to reach the same logic.
...
- The encode writes to `{stem}-chromecast.part.mp4` and is atomically renamed to the final `-chromecast.mp4` only after `_validate_output` passes, so a scan can never mistake a partially-written file for a finished output (which previously queued spurious verifies).
- `FFmpegBuilder` selects decode/scale/encode stages (NVDEC/NPP/NVENC first) and toggles HDR tonemapping (`tonemap_cuda` when supported, CPU filters otherwise). GPU decode uses the explicit `{codec}_cuvid` NVDEC decoder when available (the generic `-hwaccel cuda` path rejects some streams), and `hwdownload` maps to the surface's native software format (`nv12`/`p010le`). Bitrate/level/profile constraints are validated in orchestrator config before workers ever see them.
- HDR tonemapping scales to the target size **before** the `zscale`/`tonemap` chain (odd source dimensions otherwise abort zscale on 4:2:0), and drops the source's mastering-display/content-light/HDR10+ frame side data (`sidedata=delete`) so the source's HDR metadata does not survive into the SDR output as `mdcv`/`clli` boxes and trip compliance.
