...
- HDR tonemapping scales to the target size **before** the `zscale`/`tonemap` chain (odd source dimensions otherwise abort zscale on 4:2:0), and the output has SEI stripped (`filter_units=remove_types=6`) so the source's mastering-display/content-light metadata does not survive into the SDR output and trip compliance.
