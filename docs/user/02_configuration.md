...
- GPU workers now parse HDR metadata (transfer characteristics, mastering data, bit depth). If a stream is HDR10/HLG or >8-bit, workers either tonemap to SDR bt709/nv12 (default) or fail fast when the profile sets `allow_tonemap: false`, and job status messages label retryable causes (device busy/incomplete moov) separately from fatal codec/pix_fmt issues.
