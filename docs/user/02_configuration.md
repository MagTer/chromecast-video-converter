...
- After a successful encode the worker exports text subtitles (Swedish/English, plus untagged tracks) as `.srt` sidecar files next to the output. Bitmap subtitles (PGS/VobSub) cannot be converted to text; when none of the source's subtitles survive (neither embedded nor as sidecars), the job message carries a warning and **originals are never deleted** — neither by `remove_original_after_success` nor by delete jobs.
