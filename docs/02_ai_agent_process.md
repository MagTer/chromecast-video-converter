...
- [ ] For API/UI changes: keep `/api/libraries` runtime add/remove, websocket message schemas (`entry-update`, `job-update`, `library-update`), `/api/library/entries` pagination (`limit`/`offset`, array responses), and watcher spool behavior in sync with docs and frontend.
